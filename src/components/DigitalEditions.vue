...
import { fetchEditionItems } from "../services/EHRIGetters.js";
...
export default {
  ...
  setup(props) {
      ...
      return {
        ...
        getUnitsOnScroll: () => {
          ...
          fetchEditionItems(
            ...
            .then((newUnits) => {
              if (
                Object.values(DigitalEditionsData.value[editionKey].filters)
                  .map((v) => v)
                  .some((v) => v !== "")
              ) {
                newUnits.data.records.forEach((newItem) => {
                  if (
                    !DigitalEditionsData.value[editionKey].filteredItems.some(
                      (item) => item.id === newItem.id
                    )
                  ) {
                    DigitalEditionsData.value[editionKey].filteredItems.push(
                      newItem
                    );
                  }
                });
                ...
                newUnits.data.records.forEach((newItem) => {
                  if (
                    !DigitalEditionsData.value[editionKey].items.some(
                      (item) => item.id === newItem.id
                    )
                  ) {
              ...
              DigitalEditionsData.value[editionKey].facets =
                newUnits.data.facets;
              DigitalEditionsData.value[editionKey].pagination["total"] =
                newUnits.data.total;
              DigitalEditionsData.value[editionKey].page++;
            ...
            .catch((error) => {
              console.error(
                `Error fetching data for the ${editionKey}:`,
                error
              );
              DigitalEditionsData.value[editionKey].loading = false;
            });
    ...
    for (const editionKey in editionsConfig) {
      DigitalEditionsData.value[editionKey] = generateEdition(
        editionKey,
        editionsConfig[editionKey]
      );
    }
    ...
    const selectEdition = (key) => {
      ...
      selectedEdition.value =
        DigitalEditionsData.value[key.target.value];
    ...
    // Watch only the items and filteredItems arrays to avoid the infinite loop
    watch(
      () =>
        Object.values(DigitalEditionsData.value).map(
          (edition) => edition.items.length + edition.filteredItems.length
        ),
      () => {
        if (
          Object.values(DigitalEditionsData.value).every(
            (v) => !v.loading
          ) &&
          (!selectedEdition.value ||
            (selectedEdition.value &&
              !selectedEdition.value["isFiltered"]))
        ) {
          loading.value = true;
          let entries = Object.entries(DigitalEditionsData.value).map(
            (v) => v
          );
          sortedDEResults.value = entries.sort(
            (a, b) =>
              b[1]["pagination"]["total"] - a[1]["pagination"]["total"]
          );
          sortedDEResultsNonNull.value = sortedDEResults.value.filter(
            (i) => {
          ...
          if (
            selectedEdition.value["items"] &&
            selectedEdition.value["items"].length
          ) {
            return false;
          } else {
            selectedEdition.value =
              DigitalEditionsData.value[
                sortedDEResultsNonNull.value[0][0]
              ];
          }
          loading.value = false;
        }
