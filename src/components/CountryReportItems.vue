...
export default {
  ...
  setup(props) {
    ...
    onMounted(()=>{
      getUnitsOnScroll()
      .then(()=>{
        selectedCountryID.value = items.value.length>0?items.value[0].id:null
      })
    })


    return { isLargeScreen, expandFocusedItem, focusedItemClass, toggleFocusedItemClass, total, items, el, loading, desc, selectedCountryID, changeCountryID };
  },
};
</script>
